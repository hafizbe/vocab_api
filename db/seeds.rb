...
suras.each do |sura|
  unless sura.empty?
    nb_versets = sura[1]
    name_arabic = sura[4]
    name_phonetic = sura[5]
    type_surah = sura[7]
    the_surah = Sura.new(:name_arabic => name_arabic, :name_phonetic => name_phonetic, :nb_versets => nb_versets, :nb_cards => 10)
    the_surah.save!
  end
end

doc = Nokogiri::XML(open("lib/Quran_words.xml"))
cards = doc.xpath("/dataroot/Quran_words")

cards.length.times do |i|

  #id =  cards[i].xpath("ID/text()")
  word = cards[i].xpath("F_Word/text()").to_s

  english_m = cards[i].xpath("F_Meaning/text()").to_s
